-- Idempotent coupon redemption event logging.
-- Supplier confirmation can be retried safely without duplicate redeemed events.
...
BEGIN
  ...
  IF p_event_type IN ('coupon_issued', 'coupon_viewed', 'navigation_started', 'coupon_redeemed') THEN
...
COMMENT ON FUNCTION public.log_coupon_event(UUID, event_type, UUID, JSONB)
  IS 'Log validated coupon lifecycle events idempotently per coupon and event type';
